// Package main is the temporal lesson `l1_retries_and_timeouts` homework scaffold for Vibe Learn.
//
// Задача: PaymentWorkflow: RetryPolicy с BackoffCoefficient/MaximumAttempts/NonRetryableErrorTypes для ChargeCard.
...
func TaskQueue() string {
	return envOr("TEMPORAL_TASK_QUEUE", "lesson-l1_retries_and_timeouts-tq")
}
...
//
// флакает 503 (транзиентная, ретраится) либо CardDeclinedError (non-retryable, валит workflow сразу)
...
func main() {
	...
	log.Printf("Vibe Learn — temporal lesson %s scaffold up", "l1_retries_and_timeouts")
