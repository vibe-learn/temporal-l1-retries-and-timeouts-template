...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l1_retries_and_timeouts-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l1_retries_and_timeouts-tq", TaskQueue())
	}
}
...
func TestIntegration(t *testing.T) {
	...
	// поведения урока «Ретраи и таймауты».
}
